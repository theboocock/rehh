scan_hh<-function(haplohh,limhaplo=2,limehh=0.05,limehhs=0.05,big_gap=0,small_gap=0,small_gap_penalty=0){
  ...
  if(limehhs<0 | limehhs>1){stop("limehhs must be between 0 and 1")}
    ...
    res_scan<-.C("r_scan_hh", 
                    ...
                    min_EHHS = as.double(limehhs),
										big_gap  = as.integer(big_gap),
										small_gap = as.integer(small_gap),
										small_gap_penalty = as.integer(small_gap_penalty)
                    )
